...
for i in range(1,4): # iterating through stage folders
    ...
    with open(log_directory+"stage_"+str(i)+'_summary.csv', mode='w') as output:
        ...
        for trial_no in range(1,11): #iterating through trail numbers
            a = np.load(log_directory+"stage"+str(i) + "/turtlebot3_burger" + "_stage_" + str(i) + "_ddpg_" + str(trial_no) + ".npy", allow_pickle=True)
            # if a[0] == 'success':
            #     success = 1
            # else:
            #     success = 0
